...
`dist\EmueraEX.exe` をゲームフォルダに置くだけです。**元から入っている exe は消さないでください。**
名前が違うので共存でき、新旧の描画を並べて比べるにはどちらも必要になります。

スクリプトも同じ方針で、元の exe には触りません。同じ名前の exe を上書きしてしまう場合
（`-Name` を変えて元と同名にしたときなど）だけ `.bak` へ退避します。
...
- 元に戻すには `EmueraEX.exe` を消すだけです。元の exe はそのまま残っています
